...
fn print_devices(device: vulkano::instance::PhysicalDevice){
    println!("", );
    ...
    println!("    API_VERSION: {:?}", device.api_version());
    println!("", );

    println!("    FEATURES:");
    if device.supported_features().geometry_shader{println!("              GEO_SHADER",);}
    if device.supported_features().tessellation_shader{println!("              Tesselation_Shader",);}
    if device.supported_features().depth_clamp{println!("              Depths_Clamp",);}
    if device.supported_features().logic_op{println!("              logic_op",);}
    println!("", );

    println!("    Queue_families", );
    for (i,queue) in device.queue_families().enumerate(){
        println!("    QUEU: {}", i );
        println!("    Queue Count: {}", queue.queues_count());
        if queue.supports_graphics(){println!("        supports_graphics", );}
        if queue.supports_compute(){println!("        supports_compute", );}
        if queue.supports_transfers(){println!("        supports_transfers", );}
    }
    println!("", );
    println!("    Driver_Version: {}", device.driver_version());
    println!("    PCI_DEVICE_ID: {}", device.pci_device_id());
    println!("    PCI_VENDOR_ID: {}", device.pci_vendor_id());
    print!("    UUID: ", );

        for i in device.uuid().iter(){
            print!("{}", i);
        }
    println!("", );
}
...
fn main() {
    ...
    for i in vulkano::instance::PhysicalDevice::enumerate(&instance){
        println!("DEVICE {}", i.index());
        print_devices(i);
    }
    ...
    impl_vertex!(Color, in_color);

    //Setup first Color buffer
    let mut color_buffer = {
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), Some(queue.family()), [
            Color {in_color: [0.0,0.0,1.0] },
            Color {in_color: [0.0,1.0,0.0] },
            Color {in_color: [1.0,0.0,0.0] }
        ].iter().cloned()).expect("failed to create buffer")
    };
...
"]
    ...
    let color_buffer_1st = {
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), Some(queue.family()), [
            Color {in_color: [0.0,0.0,1.0] },
            Color {in_color: [0.0,1.0,0.0] },
            Color {in_color: [1.0,0.0,0.0] }
        ].iter().cloned()).expect("failed to create 1st color buffer")
    };

    let vertex_buffer_1st = {
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), Some(queue.family()), [
            Vertex { position: [-0.5, -0.25]},
            Vertex { position: [0.0, 0.5]},
            Vertex { position: [0.25, -0.1]}
    ...
    let color_buffer_2nd = {
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), Some(queue.family()), [
            Color {in_color: [1.0,0.0,1.0] },
            Color {in_color: [0.0,1.0,1.0] },
            Color {in_color: [1.0,1.0,0.0] },

            Color {in_color: [1.0,0.0,0.0] },
            Color {in_color: [1.0,0.0,1.0] },
            Color {in_color: [1.0,1.0,0.0] },
        ].iter().cloned()).expect("failed to create 2nd color buffer")
    };

    let vertex_buffer_2nd = {
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), Some(queue.family()), [
    ...
    loop {
        ...
        if iterations >= needed_iterations{
            //reset iterations
            iterations = 0;
            //Change to second if first
            if current_buffer == 0{
                color_buffer = color_buffer_2nd.clone();
                vertex_buffer = vertex_buffer_2nd.clone();
                current_buffer = 1;
            //change to first if second
            }else if current_buffer == 1{
                color_buffer = color_buffer_1st.clone();
                vertex_buffer = vertex_buffer_1st.clone();
                current_buffer = 0;
            }

        }else{
            //If not needed just count +1
            iterations += 1;
        }
        ...
        let command_buffer = AutoCommandBufferBuilder::new(device.clone(), queue.family()).unwrap()
            ...
            .draw(pipeline.clone(), DynamicState::none(), (vertex_buffer.clone(), color_buffer.clone()), (), ())
            .unwrap()
